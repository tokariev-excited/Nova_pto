-- ============================================================
-- Migration: Add half-day period support
-- Adds start_period, end_period, and total_days columns to
-- time_off_requests. Updates both RPC functions to handle
...
-- ==========================================================
-- 1. Add columns to time_off_requests
-- ==========================================================

ALTER TABLE time_off_requests
  ADD COLUMN IF NOT EXISTS start_period text NOT NULL DEFAULT 'morning',
  ADD COLUMN IF NOT EXISTS end_period text NOT NULL DEFAULT 'end_of_day',
  ADD COLUMN IF NOT EXISTS total_days double precision;

-- Constraints on period values
ALTER TABLE time_off_requests
  ADD CONSTRAINT chk_start_period CHECK (start_period IN ('morning', 'midday')),
  ADD CONSTRAINT chk_end_period CHECK (end_period IN ('midday', 'end_of_day'));

-- Backfill total_days for existing rows (full-day semantics)
...
-- ==========================================================
-- 2. Replace create_time_off_record with period-aware version
-- ==========================================================
...
-- ==========================================================
-- 3. Replace approve_time_off_request with period-aware version
-- ==========================================================
...
BEGIN
  ...
  -- Calculate total days using stored periods
  v_date_diff     := (v_request.end_date - v_request.start_date);
